...
su -l "$LOGIN" bash -c "mkdir -p $DEV && mkdir -p $WORKDIR && cd $DEV && git clone https://github.com/mcrapet/plowshare/ plowshare4 && sed -i -e 's/umask 0066/umask 0022/' plowshare4/src/download.sh && echo 'export TEMP_DIR=$TEMPDIR' >> $HOME_USER/.bashrc && echo 'cd $WORKDIR' >> $HOME_USER/.bashrc"
